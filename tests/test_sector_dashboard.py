...
class SectorDashboardTests(unittest.TestCase):
    ...
    def test_sector_endpoints_and_selector(self):
        with tempfile.TemporaryDirectory() as tmp, patch.dict("os.environ", {"NERIAS_DATA_SOURCE":"local"}, clear=False):
            ...
            self.assertIn("Selecione o setor", index_html)
            self.assertIn("selectSector('varejo')", index_html)
            self.assertIn("selectSector('tecnologia')", index_html)
            ...
            retail=client.get("/api/data?sector=varejo").get_json()
            technology=client.get("/api/data?sector=tecnologia").get_json()
            ...
            self.assertEqual(retail["sector_label"], "Varejo")
            self.assertFalse(retail["operational_enabled"])
            self.assertEqual(retail["operational_metrics"], [])
            self.assertEqual(retail["operational_coverage"]["status"], "not_applicable")
            self.assertEqual(technology["sector_label"], "Tecnologia")
            self.assertEqual(set(technology["tickers"]), {"BMOB3", "BRQB3", "CASH3", "ECOM3", "IFCM3", "INTB3", "LWSA3", "MLAS3", "PDTC3", "POSI3", "QUSW3", "TOTS3", "WDCN3"})
            self.assertFalse(technology["operational_enabled"])
            html = client.get("/").get_data(as_text=True)
            self.assertIn('operacional: "Dados Operacionais"', html)
            self.assertIn("Sem dados operacionais publicados.", html)
            self.assertIn("Setor exclusivamente financeiro; componentes operacionais são omitidos.", html)
